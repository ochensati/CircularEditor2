...
{
     [Serializable]
    public class TwoLines : aDecoration
    {
         public override aDecoration HandlesEngLetter(engLetter letter, Circular.aCircleObject.ScriptStyles scriptStyle)
        {
             if (scriptStyle == Circular.aCircleObject.ScriptStyles.Ashcroft)
            {
             ...
             {
                 if ("_h_p_w_x_".Contains("_" + letter.Consonant + "_"))
                     return new TwoLines();
                 else
                     return null;
            ...
            try
            {
                ...
                switch (_Location)
                {
                    ...
                    case DecorationLocation.Right:
                        {
                            ...
                            _DecorationDots.Add(new DecorationDot(p1, DecorationDot.Symbols.Anchor, 2, _Syllable));
                            _DecorationDots.Add(new DecorationDot(p2, DecorationDot.Symbols.Anchor, 3, _Syllable));


                            _DecorationDots.Add(new DecorationDot(DecorationDot.Symbols.Diamond, p1, (float)(arcMidAngle - gapAngle-90), 8));
                            _DecorationDots.Add(new DecorationDot(DecorationDot.Symbols.Diamond, p2, (float)(arcMidAngle-90), 8));
                           

                            _Sources.Add("LineSmall", new DecorationAnchor(new Point[] { p1, p3 }, .2, 2, this));
                    ...
                    case DecorationLocation.Center:
                        {
                            double aMiddle = _Syllable.SubStartAngle + _Syllable.SubArc / 2 + 90;

                            double aR = _Syllable.LetterRadius * .05;


                            Point p1 = MathHelps.D2Coords(arcX, arcY, arcRadius, aMiddle);
                            Point p2 = MathHelps.D2Coords(arcX, arcY, -1 * arcRadius, aMiddle);

                            Point p3 = MathHelps.D2Coords(arcX, arcY, arcRadius * 1.3, aMiddle);
                            Point p4 = MathHelps.D2Coords(arcX, arcY, arcRadius * -1.3, aMiddle);
